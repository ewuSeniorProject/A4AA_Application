...
namespace A4AA_Application.SurveyPages
{
	...
	public partial class SurveySectionJ : ContentPage
	{
		...
		private CommunicationT Table;

		public SurveySectionJ (Survey theSurvey)
		{
			this.theSurvey = theSurvey;
			InitializeComponent();
			Title = "Communication Technologies & Customer Service";

			var layout = this.FindByName<StackLayout>("theStackLayoutJ");
			QuestionLabels = new ArrayList();
			QuestionAnswerSpaces = new ArrayList();

			Table = theSurvey.SectionJ.CommunicationT;

			PropertyInfo[] properties = typeof(CommunicationT).GetProperties();
